...
boolean fifoReady(){
  Wire.beginTransmission(MPU_ADDR); //De quem é solicitado os dados;
  Wire.write(0x72); //Endereço no dispositivo solicitado. Refere-se a ???
  Wire.endTransmission();
  Wire.beginTransmission(MPU_ADDR); //Continuando para receber os dados...
  Wire.requestFrom(MPU_ADDR,2);//A posição de memória que será lida foi escrita anteriormente em MPU_ADDR; e o número 2 é a quantidade de bytes que é solicitado a leitura
  byte fifoCountH = Wire.read(); //Lê o byte mais significativo
  fifoCountL = Wire.read(); //Lê o byte menos significativo
  if(fifoCountL == 42 || fifoCountL == 44){ //Por que isso?
    return 1;
  }
  else return 0;
}
